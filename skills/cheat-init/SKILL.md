...
```
🎯 cheat-on-content / 网红作弊器 — 初始化

这个工具的**方法论**给所有想把"感觉"变成可校准预测的内容创作者用——
打分 → 盲预测 → 复盘 → 进化 rubric 这一套**循环本身**适用任何能被量化的内容：
视频 / 文章 / 播客 / Newsletter / 短文 thread——什么形态都行。

**rubric 是循环的"内容"，不是循环本身**。当前内置的 rubric 是观点视频版
（参考博主 25+ 视频拟合）——其他形态可以借这套**起步**，跑几篇后用
`/cheat-bump` 调整权重 + 加减维度 → 适配你的形态。

诚实告诉你三件事：
1. 你**没发过视频** → 我强烈建议你先找一个对标账号，导入几条样本（5-10 条），
   工具的初始信号（rubric / pattern / 选题方向感）从对标账号反推。
   不找对标的话用通用 v0 等权 rubric 起步，前 5 篇精度更差。
2. 你**已发过视频** → 你可以用自己的历史 calibrate，也建议至少 1 个对标做 sanity check。
3. 早期预测精度差是数学事实——通过显示 confidence 等级（🔴/🟠/🟡/🟢/🔵）告诉你
   这次预测有多可信，**不是**通过隐瞒数字。
4. 你跳过任何一次复盘，整个系统退化为占星——这是必走的纪律。
