...
下面这些路径 / 模式 sub-agent **绝不能 Read** —— 即使主 Claude 在 Task prompt 里手滑塞进来，也要拒绝并在 JSON 输出标 `refusal: "blocked_contaminated_input"`：

| 路径模式 | 为什么禁 |
|---|---|
| `.cheat-state.json` | 含 calibration_samples / pending_retros / last_published_at / shoots — 全是后视数据 |
| `predictions/*.md` | 含 `## 预测` 段 + `## 复盘` 段，复盘段就是实绩 |
| `videos/*/report.md` | T+3d 抓回的真实数据 |
| `videos/*/script.md` | 后改拍摄稿，复盘时被对照 |
| `STATUS.md` | cheat-status 渲染的看板，含过去数据 |
| `.cheat-cache/usage.jsonl` | 行为 log |
| 任何含"播放 / 阅读 / 点赞 / 评论数 / 转发 / w / 万 / k / M"的文件 | 直接污染 |

**白名单只有两个**：
- `scripts/<id>.md`（pre-shoot 草稿，传入参数）
- `rubric_notes.md`（评分公式 + 维度定义）
